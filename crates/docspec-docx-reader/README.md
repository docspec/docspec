...
- Run color properties (`<w:rPr>`):
  ...
  - `<w:shd w:val="..." w:color="..." w:fill="...">` — shading, used as a fallback highlight when `<w:highlight>` is absent. The visible color is resolved per ECMA-376 §17.3.5 / §17.18.78: for `w:val` of `nil` or `clear` (No Pattern) the visible color is `w:fill` alone; for `w:val="solid"` (100% Fill Pattern) `w:color` covers the area (falling back to `w:fill` when absent); for all other patterns (`pct10`…`pct95`, `*Stripe`, `*Cross`, etc.) the rendered color is approximated by `w:fill`. A missing `w:val` is malformed per the XSD and is defensively treated as `clear`. Emitted as `StartTextStyle { kind: Mark(Color::Rgb { r, g, b }) }`. An effective color of `"auto"` (consumer default per spec) or a missing attribute is silently dropped.
...
- Lists (`<w:p>` with `<w:numPr>`): ordered (Decimal/LowerAlpha/UpperAlpha/LowerRoman/UpperRoman) and unordered (Disc) — emitted as `Start*ListItem`/`End*ListItem` with `id` (numId stringified), `level` (ilvl), `start`, and `style_type`. Per-level classification: same numId can mix ordered and unordered levels. Any plain paragraph without `<w:numPr>` (including empty paragraphs) closes the open list immediately and emits as a top-level paragraph; styled non-numbered paragraphs still emit their dedicated `StartHeading`, `StartBlockQuote`, or `StartPreformatted` events instead. Counter continuity within a single `numId`/`ilvl` is preserved across intervening paragraphs per OOXML §17.9.17: the counter increments for each paragraph at that level "sequential or not". The first item of a `numId`/`ilvl` emits `start: Some(1)`. Items that immediately follow the previous item at the same level with no intervening non-list content emit `start: None` (sequential continuation). Items that resume after any break (table, paragraph, heading, or other non-list content between two items at the same level) emit `start: Some(N)` where N is the effective counter value. Two list items with different `numId`s at the same level produce two distinct lists, each with its own counter starting at 1. The list also closes at a heading, block quote, preformatted block, table boundary, table-cell boundary, or end of document.
...
- No `<w:start>` element parsing — `start` comes from the effective per-`numId`/`ilvl` paragraph counter, not explicit OOXML start overrides
